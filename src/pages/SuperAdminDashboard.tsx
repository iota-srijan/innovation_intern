...
import {
  ...
  Plus, X, RefreshCw, Search, Download, FileSpreadsheet, Timer,
} from 'lucide-react'
...
type Section = 'dashboard' | 'pending' | 'allreqs' | 'users' | 'audit'
...
export async function exportCustomRangeReport(fromDate: string, toDate: string) {
  const [issueRes, auditRes, inventoryRes] = await Promise.all([
    supabase
      .from('issue_requests')
      .select('*')
...
export default function SuperAdminDashboard() {
  ...
  const handleExportCustomRange = async () => {
    ...
    try {
      ...
      const [auditRes, issueRes, inventoryRes] = await Promise.all([
        supabase
          .from('audit_log')
          .select('*')
      ...
      toast.success(`Exported ${auditRows.length} audit entries, ${issueRows.length} requests, ${inventoryRows.length} inventory items`)
    } catch {
                ...
                {issueLoading ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-[12px]">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-white/[0.08]">
                          {['Student', 'Item', 'Qty', 'Professor Email', 'Reason', 'Type', 'Submitted', 'Actions'].map(h => (
                            <th key={h} className="px-2 pb-3 text-[10px] font-semibold uppercase tracking-widest text-gray-500 dark:text-[#6e6e78] first:pl-0 whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
